...
USER_CONFIGS_FILE = "user_configs.txt"  # Файл для хранения информации о конфигах пользователей
...
def load_user_configs():
    """Загружает информацию о созданных пользователями конфигах"""
    if os.path.exists(USER_CONFIGS_FILE):
        with open(USER_CONFIGS_FILE, 'r') as f:
            return {int(line.split()[0]): line.split()[1] for line in f if line.strip()}
    return {}

def save_user_config(user_id: int, config_name: str):
    """Сохраняет информацию о созданном конфиге пользователя"""
    configs = load_user_configs()
    configs[user_id] = config_name
    with open(USER_CONFIGS_FILE, 'w') as f:
        for uid, name in configs.items():
            f.write(f"{uid} {name}\n")

def remove_user_config(user_id: int):
    """Удаляет информацию о конфиге пользователя"""
    configs = load_user_configs()
...
def delete_client(client_name: str) -> bool:
    try:
        ...
        # Удаляем запись о конфиге пользователя
        configs = load_user_configs()
        user_id = next((uid for uid, name in configs.items() if name == client_name), None)
        if user_id:
...
async def request_access(message: types.Message):
    ...
    if user_id in access_requests:
        await message.answer("⏳ Ваш запрос уже отправлен и ожидает рассмотрения")
        return
...
async def start_create_config(message: types.Message, state: FSMContext):
    ...
    if message.from_user.id in user_configs:
        await message.answer(
            f"⚠️ У вас уже есть конфиг: {user_configs[message.from_user.id]}\n"
            "Для создания нового нужно удалить старый (обратитесь к администратору).",
            reply_markup=get_main_keyboard(message.from_user.id)
        )
        return
    ...
    await state.set_state(AdminStates.waiting_for_config_name)

@dp.message(AdminStates.waiting_for_config_name)
async def process_config_name(message: types.Message, state: FSMContext):
    try:
        ...
        # Сохраняем информацию о созданном конфиге
        save_user_config(message.from_user.id, name)
        await state.clear()
